...
let groupAllData = await getAllCountryMemberships()
console.log("All Data",groupAllData)
...
function filterByIsoAndMaxCount(data, isoCode, maxCount) {
  return data.filter(item =>
    item.memberCountries.includes(isoCode) &&
    item.memberCount <= maxCount
  );
}
const filteredGroupAllData = filterByIsoAndMaxCount(groupAllData, selectedCountryISO, maxParticipants);
console.log("Filtered Data", filteredGroupAllData);

function getAllUniqueMemberCountriesExcluding(data, excludeCode) {
  const allCountries = data.flatMap(org => org.memberCountries);
  const unique = new Set(allCountries.filter(code => code !== excludeCode));
  return unique;
}

const allUniqueMemberCountries = getAllUniqueMemberCountriesExcluding(filteredGroupAllData, selectedCountryISO);
console.log("All Unique Member Countries", allUniqueMemberCountries);
...
function showOnlyTreatyArcs(treatyName, treatyGroups, selectedFeature, geoData) {
  const { centroid } = selectedFeature.properties;
  const members = treatyGroups[treatyName] || [];
  const featureByIso = Object.fromEntries(geoData.features.map(f => [f.properties.iso_a3, f]));

  const arcs = members
    .filter(iso => iso !== selectedCountryISO && featureByIso[iso]?.properties?.centroid)
    .map(iso => ({
      source: centroid,
      target: featureByIso[iso].properties.centroid
    }));
  ...
  const countyLayer = new GeoJsonLayer({
    ...
    getFillColor: f => f.properties.iso_a3 === selectedCountryISO ? [158, 154, 200, 160] : [203, 201, 226, 120],
    getLineColor: () => [158, 154, 200, 255],
    lineWidthMinPixels: 1,
    onClick: info => renderLayers(geoData, info.object)
  });

  deckgl.setProps({ layers: [countyLayer, arcLayer] });
}

function showAllTreatyArcs(treatyGroups, selectedFeature, geoData) {
  const layers = [];
  ...
  for (const treaty in treatyGroups) {
    const members = treatyGroups[treaty] || [];
    const arcs = members
      .filter(iso => iso !== selectedCountryISO && featureByIso[iso]?.properties?.centroid)
      .map(iso => ({
        source: centroid,
...
async function renderLayers(data, selectedFeature) {
  ...
  container.innerHTML = `
    ...
    ${sortedTreaties.map(t => `
      <div class="treaty-label" data-treaty="${t.treaty}">
        <strong>${t.treaty}</strong>: ${t.count}
      </div>
    `).join('')}
  `;

  const labels = document.querySelectorAll(".treaty-label");

  labels.forEach(label => {
    const treatyName = label.dataset.treaty;

    label.addEventListener("mouseover", () => {
      showOnlyTreatyArcs(treatyName, treatyCountryGroups, selectedFeature, currentGeoData);
    });

    label.addEventListener("mouseout", () => {
      showAllTreatyArcs(treatyCountryGroups, selectedFeature, currentGeoData);
    });
  });
